...
| Human-only decision | Labelled demo waits; authenticated submitter approves exact payload | Agent approval tool absent; missing human session; stale or modified payload; foreign owner; repeat decision | Human session at form boundary; agent submit/read/cancel only | Decision tied to immutable operation version and principal, checked again on effect | Browser form CSRF/session contract; no assertion that generic preview token proves human intent | Route denial proof and production browser approval flow |
