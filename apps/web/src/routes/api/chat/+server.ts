...
export const GET: RequestHandler = async ({ locals }) => {
  ...
  const state = await getTenantChatState(membership).catch(mapTenantChatError);
  return json(state);
};
...
export const POST: RequestHandler = async ({ locals, request }) => {
  ...
  const result = await sendTenantChatMessage(membership, message).catch(mapTenantChatError);
