...
describe("withActiveTenant", () => {
  it("rejects a missing tenant instead of selecting the demo tenant", async () => {
    await expect(withActiveTenant(undefined, async () => null)).rejects.toThrow(
      "An explicit active tenant context is required.",
    );
